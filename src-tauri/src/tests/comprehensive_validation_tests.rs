//! Comprehensive validation tests for all Tauri command handlers
//! 
//! This test suite covers input validation for all major command modules,
...
use crate::models::ai_card::AIResponseCard;
...
// ===== LOCATIONS COMMAND TESTS =====

#[tokio::test]
async fn create_location_happy_path() {
    setup("test_create_location_ok").await;
    ...
    let req = commands::locations::CreateLocationRequest {
        ...
        description: Some("A hidden valley in Middle-earth".to_string()),
        location_type: Some("city".to_string()),
        geography: Some("Valley surrounded by mountains".to_string()),
    ...
    let location = resp.data.unwrap();
    assert_eq!(location.name, "Rivendell");
    assert_eq!(location.project_id, project.id);
}
...
#[tokio::test]
async fn update_location_rejects_oversized_metadata() {
    setup("test_update_location_metadata").await;

    // Create project and location first
    let proj_req = commands::projects::CreateProjectRequest {
        name: "Metadata Test Project".to_string(),
        description: None,
        genre: None,
        target_word_count: None,
    };
    ...
    let loc_resp: CommandResponse<Location> = commands::locations::create_location(loc_req).await;
    assert!(loc_resp.success);
    let location = loc_resp.data.unwrap();

    // metadata > 5,000 bytes should be rejected
    let huge_metadata = "x".repeat(5_001);
    let upd_req = commands::locations::UpdateLocationRequest {
        id: location.id,
        name: None,
        description: None,
        location_type: None,
        ...
        significance: None,
        visibility: None,
        metadata: Some(huge_metadata),
    };

    let resp: CommandResponse<()> = commands::locations::update_location(upd_req).await;
    assert!(!resp.success, "expected oversized metadata to be rejected");
    let err = resp.error.unwrap_or_default().to_lowercase();
    assert!(
        err.contains("size") || err.contains("limit") || err.contains("exceeds"),
        "unexpected error: {}",
...
// ===== STORY BIBLE COMMAND TESTS =====

#[tokio::test]
async fn create_story_bible_happy_path() {
    setup("test_create_story_bible_ok").await;
    ...
    let req = commands::story_bible::CreateOrUpdateStoryBibleRequest {
        project_id: project.id.clone(),
        braindump: Some("Initial story ideas and concepts".to_string()),
        synopsis: Some("A tale of adventure and discovery".to_string()),
        genre: Some("Fantasy".to_string()),
        style: Some("Third person limited".to_string()),
        style_examples: Some("Example prose showing the writing style".to_string()),
        pov_mode: Some("global".to_string()),
        global_pov: Some("third".to_string()),
        global_tense: Some("past".to_string()),
        global_character_pov_ids: Some(vec!["char1".to_string(), "char2".to_string()]),
    };

    let resp: CommandResponse<StoryBible> = commands::story_bible::create_or_update_story_bible(req).await;
    assert!(resp.success, "expected success, got error: {:?}", resp.error);
    let story_bible = resp.data.unwrap();
    assert_eq!(story_bible.project_id, project.id);
    assert_eq!(story_bible.genre, Some("Fantasy".to_string()));
}
...
#[tokio::test]
async fn create_character_trait_rejects_oversized_value() {
    setup("test_character_trait_oversized").await;

    // trait_value > 5,000 bytes should be rejected
    let huge_value = "x".repeat(5_001);
    let req = commands::story_bible::CreateCharacterTraitRequest {
        character_id: "test_char_id".to_string(),
        trait_name: "Personality".to_string(),
        trait_value: huge_value,
        visibility: None,
    };

    let resp: CommandResponse<CharacterTrait> = commands::story_bible::create_character_trait(req).await;
    assert!(!resp.success, "expected oversized trait_value to be rejected");
    let err = resp.error.unwrap_or_default().to_lowercase();
    assert!(
        err.contains("size") || err.contains("limit") || err.contains("exceeds"),
        "unexpected error: {}",
...
#[tokio::test]
async fn create_folder_rejects_invalid_order_index() {
    setup("test_create_folder_order").await;
...
// ===== AI CARDS COMMAND TESTS =====

#[tokio::test]
async fn get_ai_response_card_rejects_malicious_id() {
    setup("test_ai_cards_malicious_id").await;

    let malicious_ids = vec![
        "<script>alert('xss')</script>".to_string(),
        "'; DROP TABLE ai_cards; --".to_string(),
        "../../../etc/passwd".to_string(),
    ];

    for bad_id in malicious_ids {
        let resp: CommandResponse<AIResponseCard> = 
            commands::ai_cards::get_ai_response_card(bad_id.clone()).await;
        assert!(!resp.success, "expected security validation to fail for: {}", bad_id);
        let err = resp.error.unwrap_or_default().to_lowercase();
        assert!(
            err.contains("validation") || err.contains("security") || err.contains("invalid"),
