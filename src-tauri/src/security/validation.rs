...
pub fn validate_email(email: &str) -> Result<(), StoryWeaverError> {
    if !EMAIL_REGEX.is_match(email) {
        return Err(StoryWeaverError::ValidationError("Invalid email address format".to_string()));
    }
    Ok(())
}
...
pub fn validate_filename(filename: &str) -> Result<(), StoryWeaverError> {
    if filename.is_empty() {
        return Err(StoryWeaverError::ValidationError("Filename cannot be empty".to_string()));
    }
    
    if !FILENAME_REGEX.is_match(filename) {
        return Err(StoryWeaverError::ValidationError("Filename contains invalid characters".to_string()));
    }
...
pub fn validate_path(path: &str) -> Result<(), StoryWeaverError> {
    if PATH_TRAVERSAL_REGEX.is_match(path) {
        return Err(StoryWeaverError::ValidationError("Path contains directory traversal sequences".to_string()));
    }
    
    // Ensure the path is within the allowed directories
    let path_obj = Path::new(path);
    if path_obj.is_absolute() {
        return Err(StoryWeaverError::ValidationError("Absolute paths are not allowed".to_string()));
    }
...
pub fn validate_api_key(api_key: &str) -> Result<(), StoryWeaverError> {
    ...
    if api_key.len() < 8 {
        return Err(StoryWeaverError::ValidationError("API key is too short".to_string()));
    }
    
    if api_key.chars().all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_') {
        Ok(())
    } else {
        Err(StoryWeaverError::ValidationError("API key contains invalid characters".to_string()))
    }
}
...
pub fn validate_project_name(name: &str) -> Result<(), StoryWeaverError> {
    if name.is_empty() {
        return Err(StoryWeaverError::ValidationError("Project name cannot be empty".to_string()));
    }
    
    if name.len() > 100 {
        return Err(StoryWeaverError::ValidationError("Project name is too long".to_string()));
    }
...
pub fn validate_document_name(name: &str) -> Result<(), StoryWeaverError> {
    if name.is_empty() {
        return Err(StoryWeaverError::ValidationError("Document name cannot be empty".to_string()));
    }
    
    if name.len() > 100 {
        return Err(StoryWeaverError::ValidationError("Document name is too long".to_string()));
    }
...
pub fn validate_folder_name(name: &str) -> Result<(), StoryWeaverError> {
    if name.is_empty() {
        return Err(StoryWeaverError::ValidationError("Folder name cannot be empty".to_string()));
    }
    
    if name.len() > 100 {
        return Err(StoryWeaverError::ValidationError("Folder name is too long".to_string()));
    }
...
pub fn validate_series_name(name: &str) -> Result<(), StoryWeaverError> {
    if name.is_empty() {
        return Err(StoryWeaverError::ValidationError("Series name cannot be empty".to_string()));
    }
    
    if name.len() > 100 {
        return Err(StoryWeaverError::ValidationError("Series name is too long".to_string()));
    }
...
pub fn validate_content_length(content: &str, max_length: usize) -> Result<(), StoryWeaverError> {
    if content.len() > max_length {
        return Err(StoryWeaverError::ValidationError(format!("Content exceeds maximum length of {} characters", max_length)));
    }
